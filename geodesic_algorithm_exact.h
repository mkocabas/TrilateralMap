...
{
	if(p->_min() < GEODESIC_INF/10.0)// && p->min >= queue->begin()->first)
	{
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	m_propagation_distance_stopped = m_queue.empty() ? GEODESIC_INF : (*m_queue.begin())->_min();
	clock_t stop = clock();
...
{
	double queue_distance = (*m_queue.begin())->_min();
																	...
																	interval_pointer source_interval)	//up to two candidates
{
	...
	if(!invert)					//in this case everything is straighforward, we do not have to invert the intervals
	{
		for(unsigned i=0; i<num_candidates; ++i)
		{
			...
			p->_min() = 0.0;					//it will be changed later on
